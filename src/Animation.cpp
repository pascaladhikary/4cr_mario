...
Animation::Animation(unsigned short i_frame_width, const std::string& i_texture_location, unsigned short i_animation_speed) :
    flipped(false),
...
void Animation::draw(sf::RenderWindow& i_window)
{
    ...
    sprite.setTextureRect((flipped == 0) ? sf::IntRect(current_frame * frame_width, 0, frame_width, texture.getSize().y)
                                        : sf::IntRect(frame_width * (1 + current_frame), 0, -frame_width, texture.getSize().y));
...
void Animation::set_position(short i_x, short i_y)
{
